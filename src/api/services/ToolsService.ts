...
import { Util, Validate } from '@/util/index.js'
import { v4 } from 'uuid'
...
export default class ToolsService {
   ...
   static async updateRoles(_id: string, roles: string[]) {
      const updated = await Models.User.updateOne({ _id, login: { $ne: 'root' } }, { roles })
         .lean()
      return updated;
   }
   ...
   static async setAvatar(user_id: string, avatar?: MultipartFile) {
      ...
      const user = await Models.User.findOneAndUpdate({ _id: user_id }, { avatar: fileName }, { avatar: 1, _id: 0 })
         .lean();
